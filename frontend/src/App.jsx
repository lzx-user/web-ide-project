...
function App() {
  ...
  // 封装一个日志小助手，利用 getState() 永远拿到最新的当前文件，无视闭包！
  const writeLog = (type, text) => {
    const currentFile = useIDEStore.getState().activeFile;  // 直接从 Zustand 仓库里拿最新的 activeFile
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      writeLog('error', '保存失败: ' + err.message);
    } finally {
      setIsSaving(false);
    }
  }
